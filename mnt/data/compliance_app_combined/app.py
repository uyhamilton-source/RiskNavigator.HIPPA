...
TAGLINE = "Cyber Risk & Compliance Intelligence"

st.set_page_config(page_title=f"{BRAND_NAME} Platform", page_icon="🛡️", layout="wide")
...
def login_view() -> None:
    st.title(f"{BRAND_NAME} Platform")
...
def render_downloads(readiness: Dict, controls_df: pd.DataFrame, selected_view: str) -> None:
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.download_button("Download scored controls CSV", data=view_df.to_csv(index=False).encode("utf-8"), file_name=f"SentinelRiskIQ_{selected_view.lower().replace(' ','_')}_scored_controls.csv", mime="text/csv")
    with col2:
        st.download_button("Download readiness JSON", data=json.dumps(readiness, indent=2), file_name=f"SentinelRiskIQ_{selected_view.lower().replace(' ','_')}_readiness_summary.json", mime="application/json")
    with col3:
        st.download_button("Download executive PDF", data=build_pdf(readiness), file_name=f"SentinelRiskIQ_{selected_view.lower().replace(' ','_')}_executive_report.pdf", mime="application/pdf")

def render_dashboard() -> None:
    st.title(f"{BRAND_NAME} Compliance Dashboard")
    st.caption(TAGLINE)
    st.markdown(f"""
    <div style="padding:12px;border-radius:12px;background:#0A1F44;color:white;text-align:center;margin-bottom:12px;">
        <h2 style="margin:0;">{BRAND_NAME}</h2>
        <p style="margin:0;font-size:12px;">{TAGLINE}</p>
    </div>
    """, unsafe_allow_html=True)
    with st.sidebar:
        st.header("Controls")
